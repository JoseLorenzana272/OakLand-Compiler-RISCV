...
export class CompilerVisitor extends BaseVisitor {
    ...
    */
    ...
    visitJoin(node) {
        ...
        this.code.li(r.T4, 10);             // Divisor = 10
        this.code.rem(r.T4, r.T2, r.T4);    // T4 = T2 % 10 (residuo)
        this.code.addi(r.T4, r.T4, 48);     // Convertir a ASCII
        
        // Guardar dígito
        this.code.sb(r.T4, r.HP);           // Guardar en heap
        this.code.addi(r.HP, r.HP, 1);      // Avanzar heap
        
        this.code.li(r.T4, 10);             // Restaurar 10 en T4
        this.code.div(r.T2, r.T2, r.T4);    // Siguiente dígito
        this.code.bnez(r.T2, digitLoop);    // Si quedan dígitos, continuar
        
        // Ahora HP apunta al final de la cadena y T3 al inicio
        // Invertir la cadena
        this.code.addi(r.HP, r.HP, -1);     // Retroceder HP al último carácter
        
        const reverseLoop = this.code.getLabel();
        const endReverse = this.code.getLabel();
        
        this.code.addLabel(reverseLoop);
        this.code.bge(r.T3, r.HP, endReverse);  // Si inicio >= fin, terminar
        
        // Intercambiar caracteres
        this.code.lb(r.T4, r.T3);           // Cargar chars
        this.code.lb(r.T2, r.HP);
        this.code.sb(r.T2, r.T3);
        this.code.sb(r.T4, r.HP);
        
        this.code.addi(r.T3, r.T3, 1);      // Avanzar puntero de inicio
        this.code.addi(r.HP, r.HP, -1);     // Retroceder puntero de final
        this.code.j(reverseLoop);
        
        this.code.addLabel(endReverse);
        
        // Restaurar T3 al inicio de la cadena
        this.code.mv(r.T3, r.HP);           // T3 vuelve al inicio
        ...
        this.code.addi(r.T4, r.T1, -1);
        this.code.bge(r.T0, r.T4, skipComma);
